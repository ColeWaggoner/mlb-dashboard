            ...
            for data in data_list:
                try:
                    for ab_id in range(len(data['liveData']['plays']['allPlays'])):
                        ...
                        # FIX: allPlays contains both genuine at-bats (result.type ==
                        # 'atBat') AND standalone non-PA "action" plays recorded as
                        # their own entries — stolen bases, caught stealing, pickoffs,
                        # mound visits, etc. (result.type == 'action'). Before this
                        # fix, an action play's trailing event could get swept up by
                        # the dropped-plate-appearance fix above (which intentionally
                        # includes an at-bat's last event even when it isn't a pitch)
                        # and get miscounted as a real plate appearance carrying
                        # whatever that action's own event/eventType was (e.g. "caught
                        # stealing 2b") — inflating PA/AB counts without a matching
                        # official plate appearance behind it. Skipping non-atBat
                        # entries here keeps the earlier fix scoped to genuine at-bats
                        # only, which is what it was actually meant to cover.
                        if ab_result.get('type') != 'atBat':
                            continue
                        ...
                        for n in range(len(ab_list['playEvents'])):
                            ...
                            # FIX: the at-bat's outcome (event_type, hit/out/etc — see below) is only
                            # ever attached to the row for the LAST playEvent in this at-bat. If that
                            # last event isn't itself a pitch (e.g. a baserunning/defensive event MLB
                            # recorded chronologically after the batter's decisive pitch), the old
                            # condition here skipped it entirely — silently dropping the whole plate
                            # appearance's result from the dataframe (undercounting AVG/OBP/SLG etc,
                            # even though the game itself and its OTHER at-bats still came through
                            # correctly, matching official games-played counts). Including the final
                            # event unconditionally guarantees every at-bat contributes exactly one
                            # outcome-bearing row.
                            is_last_event_of_ab = n == len(ab_list['playEvents']) - 1
                            
                            if is_pitch_or_call or is_walk or is_last_event_of_ab:
