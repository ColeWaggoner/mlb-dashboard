...
st.set_page_config(page_title="Team Dashboard", layout="wide", initial_sidebar_state="expanded")
...
with st.sidebar:
    ...
    st.divider()
    st.caption(
        "Team stats come from bulk league-wide endpoints (one request covers "
        "every team), so this loads fast regardless of roster size. Statcast "
        "rollups are an average across that team's own qualified players in "
        "the Baseball Savant pool, not the full active roster."
    )
...
if st.session_state.team_meta is None:
    theme.page_header("Team Dashboard", "Pick a team to get started", "", ACCENT)
    st.markdown(
...
hitting_pool = st.session_state.team_hitting_pool
pitching_pool = st.session_state.team_pitching_pool
is_mlb = meta["sport_id"] == 1

if (hitting_pool is None or hitting_pool.empty) and (pitching_pool is None or pitching_pool.empty):
    theme.page_header(meta["level"], meta["name"], meta.get("league_name") or "", ACCENT, badge=meta["level"])
    st.info("No team stats loaded — see the warnings above for why, or try Force refresh.")
    st.stop()

theme.page_header(meta["level"], meta["name"], f"{meta.get('league_name','')} · {season} season",
                   ACCENT, badge=meta["level"])
...
st.caption(f"Percentiles vs. all {len(hitting_pool) if hitting_pool is not None else 0} teams at the {meta['level']} level this season.")
...
with st.container(border=True):
    ...
    roster = st.session_state.team_hitters_roster
...
st.caption(f"Percentiles vs. all {len(pitching_pool) if pitching_pool is not None else 0} teams at the {meta['level']} level this season.")
...
with st.container(border=True):
    ...
    roster = st.session_state.team_pitchers_roster
    if roster is not None and not roster.empty:
...
st.caption(
    "Everything above loads from bulk season-total endpoints. The charts below need every pitch "
    "thrown to and by this team all season, so they're pulled separately, on demand — this is a "
    "much bigger request (up to 150+ games) and can take a few minutes the first time. Cached "
...
if pitch_log is None:
    ...
    team_batting_df = pitch_log[pitch_log["batter_team_id"] == meta["team_id"]]
    team_pitching_df = pitch_log[pitch_log["pitcher_team_id"] == meta["team_id"]]
    ...
    with dd_tab_contact:
        ...
        st.caption("Pull/Center/Opposite (used internally here) is computed per batter using each hitter's own "
                   "handedness, so mixed lineups classify correctly.")

    with dd_tab_trend:
        run_trend = stats.compute_team_run_trend(pitch_log, meta["team_id"], data_layer.get_schedule_lookup(int(season), meta["sport_id"]))
        st.plotly_chart(charts.team_run_trend_chart(run_trend, window=10), width="stretch")
        st.caption("The single clearest 'hot or cold' signal — rolling runs scored vs. allowed per game. "
                   "Requires the live feed's running score fields; if this comes up empty, the debug "
                   "expander below will show why.")
